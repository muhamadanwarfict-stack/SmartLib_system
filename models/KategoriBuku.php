...
class KategoriBuku {
    ...
    public function create() {
        $query = "INSERT INTO " . $this->table_name . "
                  SET kode_kategori=:kode_kategori, nama_kategori=:nama_kategori, deskripsi=:deskripsi";

        $stmt = $this->conn->prepare($query);

        $this->kode_kategori = htmlspecialchars(strip_tags($this->kode_kategori));
        $this->nama_kategori = htmlspecialchars(strip_tags($this->nama_kategori));
        $this->deskripsi = htmlspecialchars(strip_tags($this->deskripsi));

        $stmt->bindParam(':kode_kategori', $this->kode_kategori);
    ...
    public function readOne() {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $this->kode_kategori = $row['kode_kategori'];
