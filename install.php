<?php
// Script instalasi database untuk Sistem Informasi Perpustakaan
error_reporting(E_ALL);
ini_set('display_errors', 1);

echo "<h1>Instalasi Database - Sistem Informasi Perpustakaan</h1>";
...
try {
    ...
    // Cek tabel lain
    $tables = ['kategori_buku', 'buku', 'anggota', 'peminjaman'];
    foreach ($tables as $table) {
        $stmt = $test_pdo->query("SELECT COUNT(*) as count FROM $table");
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        echo "<p>✅ Tabel '$table': " . $result['count'] . " record</p>";
    }
    ...
    echo "<ul>";
    echo "<li>Pastikan MySQL service sudah running (XAMPP Apache & MySQL)</li>";
